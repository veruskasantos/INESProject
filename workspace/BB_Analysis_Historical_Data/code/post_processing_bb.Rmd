...
```{r read}
gps_shape_data = read.csv(here::here("data/outputBusBunching/outputBusBunching-allData.csv"))
...
```{r}
saveCSV(bb_per_route, 'bb_per_route-allData.csv')
...
bb_per_bus <- route_bus_trip_bb %>%
  ...
  group_by(busCode, hasBusBunching) %>%
  count(trips = n(), percTrips = trips/totalTrips) %>% #total of trips with bb
  subset(select = -c(n))
...
```{r}
saveCSV(bb_per_bus, 'bb_per_bus-allData.csv')
...
bb_per_hour = gps_shape_data %>%
  mutate(hasBB = busBunching != "")  %>%
  group_by(route, tripNum, busCode, time, hasBB) %>%
  count()

# Removing column total of bus bunching because we want at least one occurrence
trip_has_BB_per_hour = bb_per_hour %>%
  group_by(route, tripNum, busCode, time, hasBB) %>%
  subset(select = -c(n))

# Bus trip with bus bunching or not
day_route_bus_trip_time_bb <- setNames(aggregate(trip_has_BB_per_hour$hasBB, by = list(trip_has_BB_per_hour$route, trip_has_BB_per_hour$busCode, trip_has_BB_per_hour$tripNum, trip_has_BB_per_hour$time), max), c("route", "busCode", "tripNum", "time", "hasBusBunching"))
...
trips_per_route_and_hour <- day_route_bus_trip_time_bb %>%
  group_by(route, time) %>%
  mutate(totalTrips = n()) %>% #total of trips
  group_by(route, time, hasBusBunching) %>%
  count(trips = n(), percTrips = trips/totalTrips) %>% #total of trips with bb
  subset(select = -c(n))
...
```{r}
saveCSV(bb_per_route_and_hour, 'bb_per_route_and_hour-allData.csv')
```