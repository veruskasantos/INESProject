...
public class JamData extends WazeData {
	...
	private String jamSpeedKM; //when there is no jam alert, the street is NORMAL, but we don't know the speed
	...
	//jamUpdateDate,jamExpirationDateTime,jamBlockType,jamDelay,jamLength,jamLevel,jamSeverity,jamSpeedKM
	public JamData(String updateDateTime, String expirationDateTime, String blockType, String delay, String length, 
			String level, String severity, String speedKMH) {
	...
	public List<Tuple2<Double, Double>> getJamLatLon() {
		...
		for (String coordinates : this.jamCoordinates) {
			List<Double> latLon = getLatLonDouble(coordinates);
	...
	public String getJamSpeedKM() {
		return jamSpeedKM;
	}

	public void setJamSpeedKM(String jamSpeedKM) {
...
//+ "jamDelay,jamLength,jamLevel,jamSeverity,jamSpeedKM,jamDistanceToClosestShapePoint"
	public static String getDefaultJam(String gps_datetime) {
		...
		String newJamData = jamUpdateDateTime + DELIMITER + jamExpirationDateTime + DELIMITER + jamBlockType + DELIMITER + jamDelay + 
				DELIMITER + jamLength + DELIMITER + jamLevel + DELIMITER + jamSeverity + DELIMITER + jamSpeedKM;
		return newJamData;
	}
